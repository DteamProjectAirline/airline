...
<%
System.out.println("---------------countryManageAddAction.jsp---------------");
...
//국가 입력 위한 쿼리(insert)
int insertCountry = CountryDAO.insertCountry(countryName);

//insertCountry 정상 실행 여부에 의한 분기
if (insertCountry == 1) {
	System.out.println("국가 신규등록에 성공하였습니다.");
	msg = URLEncoder.encode("국가 신규등록에 성공하였습니다.", "UTF-8");
	response.sendRedirect("/D_airline/emp/countryManage.jsp?msg=" + msg);

} else {
	System.out.println("국가 신규등록에 실패하였습니다.");
	msg = URLEncoder.encode("국가 신규등록에 실패하였습니다.", "UTF-8");
	response.sendRedirect("/D_airline/emp/countryManage.jsp?msg=" + msg);
	return;
}
%>
