...
<title>SeatManage</title>
		...
		<body>
			<h1>좌석DB관리</h1>
				...
				<form action="/D_airline/emp/seatManageSelectAction.jsp">
					<input type="number" placeholder="search flight ID(FL)"
						name="flightId">
	...
	%>
							...
							<div>
								<a
								href="/D_airline/emp/seatManageModifyAction.jsp?seatId=<%=seatId%>&flightId=<%=flightId%>&seatNo=<%=seatNo%>"><%=seatGrade%>/<%=seatNo%>/<%=seatState%>
								</a>
							</div>
